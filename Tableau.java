...
public class Tableau {
	 ...
	 * ( than go out of bounds, its direction changes and it moves down. Magnitude
	 * of the deltas are given by 0, FLEET_MOVE_X, and FLEET_MOVE_Y.
	 */
	public void moveEnemyFleet() {
		...
		if (this.getRightMostEnemy().getPosition().getX() == SpaceInvaders.WIDTH) {
			translateEnemyFleet(0, FLEET_MOVE_Y);
			fleetMovePositiveX = false;
		}
		if (this.getLeftMostEnemy().getPosition().getX() == 0) {
			translateEnemyFleet(0, FLEET_MOVE_Y);
			fleetMovePositiveX = true;
		}
